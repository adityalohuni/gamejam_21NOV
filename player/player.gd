...
func _update_animation() -> void:
	...
	# crouch when holding down on the floor
	if Input.is_action_pressed("ui_down") and is_on_floor():
		target_anim = "crouching"
	# jumping/falling animation while airborne
	elif not is_on_floor():
		target_anim = "jumping"
	# running when moving horizontally on the ground
	...
	# Only change animation if it's different, to avoid restarting frames
